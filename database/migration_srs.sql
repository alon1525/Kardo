-- Migration to Spaced Repetition System (SRS)
...
-- Step 3: Create user_progress table for SRS
-- Note: user_id type matches the users table (VARCHAR(255) for custom users, UUID for Supabase auth.users)
CREATE TABLE IF NOT EXISTS user_progress (
    user_id VARCHAR(255) NOT NULL,  -- Change to UUID if using Supabase auth.users
    ...
    -- Note: Foreign key to users table removed to support both custom users and Supabase auth.users
    -- If using Supabase auth.users, change user_id to UUID and remove the foreign key constraint
);
...
CREATE POLICY "Users can view cards in their decks" ON cards_new
    FOR SELECT USING (
        EXISTS (
            SELECT 1 FROM decks 
            WHERE decks.id = cards_new.deck_id AND decks.user_id = auth.uid()::text
        )
    );
...
-- RLS Policies for user_progress (VARCHAR version - for custom users table)
...
-- Step 6: Update card_count in decks based on actual cards
UPDATE decks 
SET card_count = (
    SELECT COUNT(*) 
    FROM cards 
...
-- Success message
SELECT 'SRS migration completed successfully! Cards table created and user_progress table ready.' AS message;
